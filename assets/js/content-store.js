...
async function article(id,options){if(!validId(id))throw new Error('Identifiant d’actualité invalide');const clean=String(id).toUpperCase(),base=RAW_ROOT+'actualites/'+clean+'/';const content=await getJSON(base+'contenu.json',options);let layout={};try{layout=await getJSON(base+'layout.json',options)}catch(_){}let media={};try{media=await getJSON(base+'media.json',options)}catch(_){}return{...content,...layout,id:clean,photos:layout.photos||content.photos||media.photos||[]}}
