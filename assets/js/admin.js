...
const modelVisualStyle=document.createElement('style');modelVisualStyle.textContent='.model-row{display:grid;grid-template-columns:repeat(2,minmax(130px,1fr));gap:12px;max-height:430px;overflow:auto;padding:4px}.model{min-width:0;min-height:122px;border:1px solid #d8e1d8;border-radius:10px;padding:8px;background:#fbfcf8;text-align:left;cursor:pointer;transition:.15s}.model:hover{border-color:#6a9a76;transform:translateY(-1px)}.model.active{border:2px solid #17603d;background:#eef7ef}.model-name{display:block;margin-top:7px;font:600 11px var(--sans);color:#315b40}.model-visual{height:78px;border-radius:6px;background:#edf1eb;padding:6px;display:grid;gap:4px}.mv-photo{background:linear-gradient(135deg,#8caf82,#d4c18f);border-radius:3px;min-height:18px}.mv-text{background:repeating-linear-gradient(to bottom,#84988a 0 2px,transparent 2px 7px);opacity:.65;border-radius:2px;min-height:20px}.mv-cols{display:grid;grid-template-columns:1fr 1fr;gap:4px}.mv-cols>i{display:block;background:#b7cdb7;border-radius:3px}.mv-gallery{display:grid;grid-template-columns:repeat(3,1fr);gap:3px}.mv-gallery i{background:#9ebd9d;border-radius:2px}.mv-quote{margin:auto;width:70%;height:17px;border-left:3px solid #6d9875;background:#d9e6d8;border-radius:2px}.mv-timeline{display:flex;gap:3px;align-items:center}.mv-timeline i{height:28px;flex:1;background:#b8cfba;border-radius:3px}.mv-onlytext{width:80%;height:45px;margin:auto;background:repeating-linear-gradient(to bottom,#8a9d90 0 2px,transparent 2px 8px)}.mv-card{background:#dbe9dc;border-radius:4px;height:30px}.mv-map{background:#c4d5c0;border-radius:50%;width:38px;height:38px;margin:auto}.mv-couple{display:grid;grid-template-columns:1fr 1fr;gap:4px}.mv-couple i{background:#c8d9c5;border-radius:3px}.mv-cta{height:18px;width:55%;margin:auto;background:#5c8d68;border-radius:10px}';document.head.appendChild(modelVisualStyle);
const modelScrollFix=document.createElement('style');modelScrollFix.textContent='.model-row{overflow-y:auto!important;overflow-x:hidden!important}';document.head.appendChild(modelScrollFix);
...
modelPreview?.addEventListener('pointerup',e=>{const item=e.target.closest('[data-model]');if(!item)return;e.preventDefault();modelSelect.value=item.dataset.model;document.querySelectorAll('.model').forEach(x=>x.classList.toggle('active',x.dataset.model===modelSelect.value));captureDraft();renderDraftPreview()});
...
buildPageTree();
async function populateRetroSeasons(){const box=document.getElementById('retroChildren');if(!box)return;try{const r=await fetch('https://raw.githubusercontent.com/shbassinchateaulin/horticulture-contenus/main/actualites/index.json');const data=await r.json();const seasons=[...new Map((data.articles||[]).filter(a=>a.status!=='deleted').map(a=>[a.seasonCode||a.season,a.season||a.seasonCode])).entries()];box.innerHTML=seasons.map(([code,label])=>`<div class="tree-item" data-season="${code}">↪ &nbsp;Saison ${label}</div>`).join('');box.querySelectorAll('[data-season]').forEach(item=>item.addEventListener('click',event=>{event.stopPropagation();document.querySelectorAll('.tree-item').forEach(x=>x.classList.remove('active'));item.classList.add('active');openLivePage('retrospective/?season='+encodeURIComponent(item.dataset.season))}))}catch(error){box.innerHTML='<div class="tree-empty">Saisons indisponibles</div>'}}
populateRetroSeasons();
// « À venir » reste une page simple : on conserve la page, sans flèche ni sous-menu.
document.querySelector('[data-toggle-folder="calendar"]')?.remove();
...
function modelVisual(kind){if(['heroGallery','heroShort','focus','portrait','reportage'].includes(kind))return'<div class="mv-photo"></div><div class="mv-text"></div>';if(['splitLeft','splitRight','advanced'].includes(kind))return'<div class="mv-cols"><i></i><div class="mv-text"></div></div>';if(['duo','trio','mosaic','galleryText','titledGrid','slideshow'].includes(kind))return'<div class="mv-gallery"><i></i><i></i><i></i></div><div class="mv-text"></div>';if(['quote','testimonials'].includes(kind))return'<div class="mv-quote"></div><div class="mv-text"></div>';if(['timeline'].includes(kind))return'<div class="mv-timeline"><i></i><i></i><i></i></div>';if(['event','faq','pdf','longform'].includes(kind))return'<div class="mv-card"></div><div class="mv-text"></div>';if(['outing'].includes(kind))return'<div class="mv-map"></div><div class="mv-text"></div>';if(['beforeAfter'].includes(kind))return'<div class="mv-couple"><i></i><i></i></div>';if(['cta'].includes(kind))return'<div class="mv-text"></div><div class="mv-cta"></div>';return'<div class="mv-onlytext"></div>'}
function updateCompatibleModels(article){const win=$('#siteFrame')?.contentWindow;const registry=win?.ArticleLayouts?.registry;if(!registry)return;const text=String(article.text||article.body||article.content||article.summary||'');const sentences=Math.max(1,(text.match(/[.!?]+/g)||[]).length);const photos=Array.isArray(article.photos)?article.photos.length:Number(article.uploadedPhotoCount||article.expectedPhotoCount||0)||(article.cover?1:0);const current=modelSelect.value;const compatible=Object.values(registry).filter(d=>photos>=d.minPhotos&&photos<=d.maxInline&&sentences>=d.minSentences&&(d.id!=='H28'||photos===0));modelSelect.innerHTML=compatible.map(d=>`<option value="${d.id}">${d.id} — ${d.name||''}</option>`).join('');if(compatible.some(d=>d.id===current))modelSelect.value=current;else if(compatible.length)modelSelect.value=compatible[0].id;modelPreview.innerHTML=compatible.map(d=>`<div class="model" data-model="${d.id}"><div class="model-visual">${modelVisual(d.kind)}</div><span class="model-name">${d.id} — ${d.name||''}</span></div>`).join('');document.querySelectorAll('.model').forEach(x=>x.classList.toggle('active',x.dataset.model===modelSelect.value))}
