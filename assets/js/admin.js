...
buildPageTree();
async function populateRetroSeasons(){const box=document.getElementById('retroChildren');if(!box)return;try{const r=await fetch('https://raw.githubusercontent.com/shbassinchateaulin/horticulture-contenus/main/actualites/index.json');const data=await r.json();const seasons=[...new Map((data.articles||[]).filter(a=>a.status!=='deleted').map(a=>[a.seasonCode||a.season,a.season||a.seasonCode])).entries()];box.innerHTML=seasons.map(([code,label])=>`<div class="tree-item" data-season="${code}">↪ &nbsp;Saison ${label}</div>`).join('');box.querySelectorAll('[data-season]').forEach(item=>item.addEventListener('click',()=>{document.querySelectorAll('.tree-item').forEach(x=>x.classList.remove('active'));item.classList.add('active');openLivePage('retrospective/?season='+encodeURIComponent(item.dataset.season))}))}catch(error){box.innerHTML='<div class="tree-empty">Saisons indisponibles</div>'}}
populateRetroSeasons();
// « À venir » reste une page simple : aucun sous-menu à afficher dans l’administration.
document.querySelector('[data-toggle-folder="calendar"]')?.closest('.tree-folder')?.remove();
