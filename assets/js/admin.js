...
function openLivePage(path){const frame=$('#siteFrame');if(frame)frame.src=new URL(path,document.baseURI).href}
...
function updateCompatibleModels(article){const win=$('#siteFrame')?.contentWindow;const registry=win?.ArticleLayouts?.registry;if(!registry)return;const text=String(article.text||'');const sentences=(text.match(/[.!?]+/g)||[]).length;const photos=Array.isArray(article.photos)?article.photos.length:0;[...modelSelect.options].forEach(option=>{const d=registry[option.value];const ok=d&&photos>=d.minPhotos&&photos<=d.maxPhotos&&sentences>=d.minSentences&&(option.value!=='H28'||photos===0);option.hidden=!ok;option.disabled=!ok});if(modelSelect.selectedOptions[0]?.disabled){const first=[...modelSelect.options].find(o=>!o.disabled);if(first)modelSelect.value=first.value}}
function selectArticle(id){selectedArticle=id;articleEditor.hidden=false;membershipEditor.hidden=true;$('#inspectorTitle').textContent='Mise en page de l’actualité';$('#breadcrumb').textContent='Pages › Actualités › '+id;openLivePage('actualites/article/?id='+encodeURIComponent(id));const draft=drafts[id];if(draft){$('#editorTitle').value=draft.title||'';$('#editorText').value=draft.text||'';modelSelect.value=draft.layout||'H28';renderDraftPreview();return}fetch('https://raw.githubusercontent.com/shbassinchateaulin/horticulture-contenus/main/actualites/'+encodeURIComponent(id)+'/contenu.json').then(r=>r.json()).then(a=>{if(selectedArticle!==id)return;loadedArticle=a;const text=a.text||a.body||a.content||a.paragraphs?.map(p=>typeof p==='string'?p:p.text||'').join('\n\n')||a.summary||'';$('#editorTitle').value=a.title||'';$('#editorText').value=text;const layout=a.layoutCode||a.layout||((a.photos||[]).length?'H04':'H28');modelSelect.value=layout;document.querySelectorAll('.model').forEach(x=>x.classList.toggle('active',x.dataset.model===layout));updateCompatibleModels(a);markDirty()}).catch(()=>{})}
